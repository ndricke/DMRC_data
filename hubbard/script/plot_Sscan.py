...
for U in U_list:
    for sig in S_list:
        data = check.read_data_Hubbard(parent_path, 10, 5, sig, U) # (parent_path, nsite, nocc, sigma, U)
        ...
        plt.scatter(pops.flatten(), pairs.flatten(), label="U=%s, sig=%s" % (U, sig))

plt.legend()
plt.ylabel("2-pdm Diagonal Elements")
plt.xlabel("1-pdm Diagonal Elements")
#plt.show()
plt.savefig("n10_no5_Sscan_Uscan_1pdmV2pdm.png", transparent=True, bbox_inches="tight", pad_inches=0.05)
